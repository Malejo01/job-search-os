...
### JS-020 · Inbound email (Resend) — configuración a medias
`deps:` JS-019 · `est:` 2 h
- **Configurado 2026-09-17:** dominio de ingesta `ingest.malejo.com.ar` con sus registros de **envío** en la zona de Vercel (MX de feedback, SPF, DKIM); webhook `https://busquedalaboral.malejo.com.ar/api/inbound` con `RESEND_WEBHOOK_SECRET` e `INGEST_DOMAIN` en producción; `profiles.inbound_address` = `u_a0000000@ingest.malejo.com.ar` por UPDATE puntual (el seed completo NO se corre contra producción). Verificado contra producción: 401 sin firma, 401 con firma inválida, 200 `unknown_recipient` con firma válida.
- **Falta (bloquea la recepción):** habilitar *Receiving* en Resend para ese dominio y cargar el **MX de recepción** sobre `ingest.malejo.com.ar` (es otro, aparte del de envío, y debe ser el de menor prioridad del subdominio). Después, el filtro de reenvío en Gmail. Pasos en `docs/DEPLOY.md`.
...
### JS-021 · Parser LinkedIn
`deps:` JS-020 · `est:` 2 h
- Fixtures de 5 emails reales anonimizados; extrae título, empresa, ubicación, modalidad, badges, aptitudes, candidatos, URL canónica. Falla cerrado.
